{{--
    ATTENTION
    =========
    /src/scss/partials/list-property.scss

    This partial is used to display your property search
    results in a list style.
--}}
        ...
        <div class="row g-0">
            <div class="col-md-5">
                <div class="property__image">
                    @if ($hasImages)
                        <img loading="lazy" src="{{ $property['images'][0]['optimised_image_url'] ?? '' }}/460" class="img-fluid rounded-start" alt="{{ $property['Address']['display_address'] ?? '' }}" data-media-update-date="{{ $property['images'][0]['media_update_date']}}" data-caption="{{ $property['images'][0]['caption'] }}">
                    @else
                        <img loading="lazy" src="" class="img-fluid rounded-start" alt="Awaiting Images for {{ $property['Address']['display_address'] }}">
                    @endif
                </div>
            </div>
            <div class="col-md-7">
